...
@pytest.fixture
def stage02_like_db(tmp_path: Path) -> Path:
...
def test_deterministic_queue_ids_and_order(stage02_like_db: Path, tmp_path: Path) -> None:
    out1 = tmp_path / "q1.json"
    out2 = tmp_path / "q2.json"
    build_stage03(stage02_like_db, out1)
    build_stage03(stage02_like_db, out2)
...
def test_missing_fa_coverage_report_and_owner_stop(stage02_like_db: Path, tmp_path: Path) -> None:
    out = tmp_path / "q.json"
    pkt = tmp_path / "pkt.json"
    rep = tmp_path / "rep.txt"
    result = build_stage03(stage02_like_db, out, packet_path=pkt, report_path=rep)
...
def test_zero_automatic_fa_jobs_and_historical_identity_rejection(stage02_like_db: Path, tmp_path: Path) -> None:  # noqa: E501
    out = tmp_path / "q.json"
    build_stage03(stage02_like_db, out)
    data = json.loads(out.read_text())
...
def test_stable_refs_not_numeric_ids(stage02_like_db: Path, tmp_path: Path) -> None:
    out = tmp_path / "q.json"
    build_stage03(stage02_like_db, out)
    data = json.loads(out.read_text())
    for it in data["items"]:
...
def test_no_network_and_input_unchanged(stage02_like_db: Path, tmp_path: Path) -> None:
    sha_before = sha256_file(stage02_like_db)
    out = tmp_path / "q.json"
    build_stage03(stage02_like_db, out)
    sha_after = sha256_file(stage02_like_db)
    assert sha_before == sha_after


def test_overwrite_refusal(stage02_like_db: Path, tmp_path: Path) -> None:
    out = tmp_path / "q.json"
