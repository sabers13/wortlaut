...
from app.resolve import resolve_word  # noqa: E402
...
def evaluate_coverage(dictionary: Dictionary, words: Sequence[str]) -> CoverageResult:
    """Evaluate resolution coverage across a sequence of normalized words.

    An entry is a hit iff at least one returned reference has status 'resolved'
    or 'derived_compound'. Misses retain exact input order.
    ...
    for entry in words:
        ...
        is_hit = any(r.status in ("resolved", "derived_compound") for r in refs)
        if is_hit:
            hits += 1
        else:
            misses.append(entry)
...
def run_gate2_coverage(
    ...
    with Dictionary(dict_p) as dictionary:
        result = evaluate_coverage(dictionary, words)
