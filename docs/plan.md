...
| slice-8 | 9 | — | **Standalone product completion, in S8A–S8E:** repair `reference/smoke_test.py` and remove reference tooling exclusions; complete two-stage capture, CSV import, deterministic example ranking, and the full ADR-0002 §4/§5, ADR-0003 §5, ADR-0004 D47 and ADR-0005 pronunciation smoke matrix; establish an authoritative Lit/TypeScript/Web Components/Vite frontend with CSS tokens and Playwright (generated `app/frontend/` is build output only); deliver standalone deck/capture/import/review/DE-EN meaning/pronunciation UX, real export-only `genanki` APKG with semantic archive/media tests, and one FastAPI-served production package; finish with Playwright E2E against that actual service plus the mandatory T3 full-diff review. No lecture app is required or integrated. |
