...
| §5 `reference/smoke_test.py` baseline asserts scalar `note.gloss_user` column effects | the pending future smoke contract asserts instead: explicit `meaning_langs` required at creation; `meaning_langs` omission/no-mutation on reuse; add DE/EN/FA user meaning; update one language without touching others; delete one language; user meaning surviving deselection/reselection; no implicit English; unsupported language → 422 / zero writes; blank user meaning → 422 / zero writes; malformed `user_meanings` → 422 / zero writes; atomic rollback of the entire `/vocab/cards` request; D10 contribution only for English |
