...
The previously pending amendments are in force. ADR-0002 D26 remains unchanged in
behavior: local Piper is authoritative; optional remote `/speak` is bounded to
total timeout <= 1 second and every remote failure falls back locally. ADR-0005
D56 makes that accepted local fallback executable by assigning slice-6 only the
image-build/runtime Piper engine+voice prerequisite. Runtime pronunciation
source selection, human/custom media, cache semantics, API and UI remain slice-7
work.
...
|---|---|
| §4 Picker / §4 Stage 2 commit: a selection carries dictionary identity plus `overrides` only | each selection additionally carries the note's **non-empty** meaning-language set drawn from `{de, en, fa}`, which the picker offers at note/card creation. Stage-1 picker payload includes an immutable dictionary asset token `(version + SHA-256)`. Resolved candidates transport stable lemma `ref` + stable `sense_ref`; derived candidates transport an ordered stable component `(lemma_ref, sense_ref)` vector; numeric IDs, if present, are convenience caches only. `/vocab/cards` round-trips the exact dictionary asset token and stable refs; if the active asset token changed between picker and commit, return HTTP 409 `dictionary_changed` before any write. Existing HTTP 422 validation and atomic commit semantics for malformed user edits remain unchanged |
...
| §4 `gloss_user` as an English-only correction for a revalidated `status='needs_gloss'` selection | scalar `gloss_user` is removed from the future override contract. User meanings are a language-keyed `user_meanings` object normalized to `note_user_meaning(note_id, language)` (D44 §6.4): string upsert / `null` delete in `/vocab/cards`; omitted object = no mutation; empty object invalid; no implicit English; all changes atomic; user meanings valid regardless of resolver status; deselecting a language does not delete authored data. Resolver `needs_gloss` stays a resolver outcome, and the derived, non-persisted `meaning_state = none | partial | complete` owns selected-language availability (D43 §6.3) |
| §10 Endpoint `POST /gloss` — "Fill `needs_gloss`, optional contribute" | superseded by `/vocab/gloss` as the language-bearing note-user-meaning edit endpoint (D44 §6.4): `POST /vocab/gloss` upserts `{note_id, language, text, contribute}` with `language ∈ {de,en,fa}`, text stored verbatim, valid regardless of resolver status; `DELETE /vocab/gloss/{note_id}/{language}` deletes only the matching `note_user_meaning` (idempotent), alters neither `meaning_langs` nor resolver `note.status`, and recomputes `meaning_state`. `contribute:true` is valid only for `language=en` (D10 English-only); `de`/`fa` contribution is HTTP 422 before any write |
...
| §5 `reference/smoke_test.py` baseline asserts scalar `note.gloss_user` column effects | the pending future smoke contract asserts instead: explicit `meaning_langs` required at creation; `meaning_langs` omission/no-mutation on reuse; add DE/EN/FA user meaning; update one language without touching others; delete one language; user meaning surviving deselection/reselection; no implicit English; unsupported language → 422 / zero writes; blank user meaning → 422 / zero writes; malformed `user_meanings` → 422 / zero writes; atomic rollback of the entire `/vocab/cards` request; D10 contribution only for English; dictionary replacement with numeric-ID renumbering where stable refs bind correctly; unrelated reuse of old numeric ID does not bind; disappeared sense fails closed to `needs_gloss` while preserving user meanings/history; duplicate/ambiguous stable refs abort activation; derived compound component disappearance renders whole derived block unavailable; `meaning_state` consults only validated current bindings; stale picker asset token → 409 and zero writes; no mixed old-binding/new-asset state observable |
...
the substance of D20–D27 (D27's wire-level dictionary-identity fields are
compatibly extended by D47, not architecturally reopened); §3's rejections;
§4.1's browser boundary and AGENTS R12; and §5's consequences other than the
note contract named above. ADR-0004 does not reopen the standalone-vs-mounted
question.
