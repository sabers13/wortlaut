...
| D9 — `needs_gloss` as "no English gloss"; "user types English during review" | ADR-0004 (pending) | `status='needs_gloss'` remains the fourth resolver fallback/stub outcome and stays independent of meaning availability: the computed `meaning_state = none | partial | complete` owns selected-language availability (D43). An unresolved note may have complete user meanings and remain resolver `needs_gloss`; a resolved note may have `meaning_state` none/partial and remain resolved. User meanings are language-bearing (`note_user_meaning`, D44). D9's card-creation behaviour and §11's "`needs_gloss` cards enter scheduling normally" rule are unchanged |
